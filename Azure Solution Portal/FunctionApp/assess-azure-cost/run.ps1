...
Write-Host "=== START assess-azure-cost ==="
...
$scriptPath = $null
$paths = @(
    (Join-Path $PSScriptRoot '..\scripts\assess-azure-cost.ps1'),
    'D:\home\site\wwwroot\scripts\assess-azure-cost.ps1',
    '/home/site/wwwroot/scripts/assess-azure-cost.ps1'
)
...
if (-not $scriptPath) {
    Push-OutputBinding -Name Response -Value ([HttpResponseContext]@{ StatusCode = 500; Body = '{"error":"Script assess-azure-cost.ps1 non trovato"}'; Headers = $corsHeaders })
    return
}
...
$outHtml = Join-Path $tempDir "cost_report_$subscriptionId.html"
$outJson = Join-Path $tempDir "cost_report_$subscriptionId.json"

if (Test-Path $outJson) { Remove-Item $outJson -Force }
if (Test-Path $outHtml) { Remove-Item $outHtml -Force }

try {
    Write-Host "Executing assess-azure-cost..."
    ...
    if (Test-Path $outJson) {
        ...
        Push-OutputBinding -Name Response -Value ([HttpResponseContext]@{ StatusCode = 500; Body = '{"error":"Report JSON Cost non generato"}'; Headers = $corsHeaders })
    }
} catch {
    ...
    Write-Host "=== END assess-azure-cost ==="
}
