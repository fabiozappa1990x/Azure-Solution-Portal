...
Write-Host "=== START assess-entra-identity ==="
...
# --- Graph token (per Microsoft Graph) ---
...
if ($subscriptionId -eq 'tenant-only') {
    Write-Host "Entra tenant-only mode: extracting tenantId from Graph JWT..."
    try {
...
$scriptPath = $null
$paths = @(
    (Join-Path $PSScriptRoot '..\scripts\assess-entra-identity.ps1'),
    'D:\home\site\wwwroot\scripts\assess-entra-identity.ps1',
    '/home/site/wwwroot/scripts/assess-entra-identity.ps1'
)
...
if (-not $scriptPath) {
    Push-OutputBinding -Name Response -Value ([HttpResponseContext]@{ StatusCode = 500; Body = '{"error":"Script assess-entra-identity.ps1 non trovato"}'; Headers = $corsHeaders })
    return
}
...
$outHtml = Join-Path $tempDir "entra_report_$subscriptionId.html"
$outJson = Join-Path $tempDir "entra_report_$subscriptionId.json"

if (Test-Path $outJson) { Remove-Item $outJson -Force }
if (Test-Path $outHtml) { Remove-Item $outHtml -Force }

try {
    Write-Host "Executing assess-entra-identity..."
    ...
    if (Test-Path $outJson) {
        ...
        Push-OutputBinding -Name Response -Value ([HttpResponseContext]@{ StatusCode = 500; Body = '{"error":"Report JSON Entra Identity non generato"}'; Headers = $corsHeaders })
    }
} catch {
    ...
    Write-Host "=== END assess-entra-identity ==="
}
