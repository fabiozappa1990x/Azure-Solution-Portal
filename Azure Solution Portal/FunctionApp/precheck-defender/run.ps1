...
Write-Host "=== START precheck-defender ==="
...
$scriptPath = $null
$paths = @(
    (Join-Path $PSScriptRoot '..\scripts\precheck-defender.ps1'),
    'D:\home\site\wwwroot\scripts\precheck-defender.ps1',
    '/home/site/wwwroot/scripts/precheck-defender.ps1'
)
...
if (-not $scriptPath) {
    Push-OutputBinding -Name Response -Value ([HttpResponseContext]@{ StatusCode = 500; Body = '{"error":"Script precheck-defender.ps1 non trovato"}'; Headers = $corsHeaders })
    return
}
...
$outHtml = Join-Path $tempDir "defender_report_$subscriptionId.html"
$outJson = Join-Path $tempDir "defender_report_$subscriptionId.json"

if (Test-Path $outJson) { Remove-Item $outJson -Force }
if (Test-Path $outHtml) { Remove-Item $outHtml -Force }

try {
    Write-Host "Executing precheck-defender..."
    ...
    if (Test-Path $outJson) {
        ...
        Push-OutputBinding -Name Response -Value ([HttpResponseContext]@{ StatusCode = 500; Body = '{"error":"Report JSON Defender non generato"}'; Headers = $corsHeaders })
    }
} catch {
    ...
    Write-Host "=== END precheck-defender ==="
}
