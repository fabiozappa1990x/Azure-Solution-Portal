...
Write-Host "=== START precheck-defender-xdr ==="
...
if (-not $authHeader -or -not $authHeader.StartsWith('Bearer ')) {
    Push-OutputBinding -Name Response -Value ([HttpResponseContext]@{ StatusCode = 401; Body = '{"error":"Token mancante"}'; Headers = $corsHeaders }); return
}
$accessToken = $authHeader.Substring(7).Trim()
if ($accessToken.Length -lt 100) {
    Push-OutputBinding -Name Response -Value ([HttpResponseContext]@{ StatusCode = 401; Body = '{"error":"Token troppo corto"}'; Headers = $corsHeaders }); return
}
...
if (-not $graphTokenHeader -or $graphTokenHeader.Length -lt 100) {
    Push-OutputBinding -Name Response -Value ([HttpResponseContext]@{ StatusCode = 401; Body = '{"error":"Graph token mancante."}'; Headers = $corsHeaders }); return
}
...
if ($subscriptionId -eq 'tenant-only') {
    ...
    } catch { Write-Host "Warning: impossibile estrarre tenantId: $($_.Exception.Message)" }
} else {
    try {
        ...
        Push-OutputBinding -Name Response -Value ([HttpResponseContext]@{ StatusCode = 401; Body = '{"error":"Token non valido o subscription non accessibile"}'; Headers = $corsHeaders }); return
    }
}
...
if ($requestedTenantId -and $graphTokenTenantId -and ($graphTokenTenantId -ne $requestedTenantId)) {
    Push-OutputBinding -Name Response -Value ([HttpResponseContext]@{ StatusCode = 403; Body = '{"error":"Il token Graph non appartiene al Tenant ID selezionato"}'; Headers = $corsHeaders }); return
}
if ($requestedTenantId -and $azureTokenTenantId -and ($azureTokenTenantId -ne $requestedTenantId)) {
    Push-OutputBinding -Name Response -Value ([HttpResponseContext]@{ StatusCode = 403; Body = '{"error":"Il token Azure non appartiene al Tenant ID selezionato"}'; Headers = $corsHeaders }); return
}
if ($requestedTenantId -and $tenantId -and ($tenantId -ne $requestedTenantId)) {
    Push-OutputBinding -Name Response -Value ([HttpResponseContext]@{ StatusCode = 403; Body = '{"error":"La subscription/tenant target non appartiene al Tenant ID indicato"}'; Headers = $corsHeaders }); return
}
if ($azureTokenTenantId -and $tenantId -and ($azureTokenTenantId -ne $tenantId)) {
    Push-OutputBinding -Name Response -Value ([HttpResponseContext]@{ StatusCode = 403; Body = '{"error":"Mismatch tra tenant del token Azure e tenant target"}'; Headers = $corsHeaders }); return
}
if ($graphTokenTenantId -and $tenantId -and ($graphTokenTenantId -ne $tenantId)) {
    Push-OutputBinding -Name Response -Value ([HttpResponseContext]@{ StatusCode = 403; Body = '{"error":"Mismatch tra tenant del token Graph e tenant target"}'; Headers = $corsHeaders }); return
}

$scriptPath = $null
$paths = @(
    (Join-Path $PSScriptRoot '..\scripts\precheck-defender-xdr.ps1'),
    'D:\home\site\wwwroot\scripts\precheck-defender-xdr.ps1',
    '/home/site/wwwroot/scripts/precheck-defender-xdr.ps1'
)
foreach ($path in $paths) { if (Test-Path $path) { $scriptPath = $path; break } }

if (-not $scriptPath) {
    Push-OutputBinding -Name Response -Value ([HttpResponseContext]@{ StatusCode = 500; Body = '{"error":"Script precheck-defender-xdr.ps1 non trovato"}'; Headers = $corsHeaders }); return
}

$tempDir = if ($env:TEMP) { $env:TEMP } else { '/tmp' }
$outHtml = Join-Path $tempDir "defenderxdr_report_$subscriptionId.html"
$outJson = Join-Path $tempDir "defenderxdr_report_$subscriptionId.json"
...
try {
    ...
    if (Test-Path $outJson) {
        ...
        Push-OutputBinding -Name Response -Value ([HttpResponseContext]@{ StatusCode = 500; Body = '{"error":"Report JSON Defender XDR non generato"}'; Headers = $corsHeaders })
    }
} catch {
    ...
    Write-Host "=== END precheck-defender-xdr ==="
}
