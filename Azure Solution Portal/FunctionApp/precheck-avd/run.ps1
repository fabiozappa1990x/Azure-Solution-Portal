...
$scriptPath = $null
$paths = @(
    (Join-Path $PSScriptRoot '..\scripts\precheck-avd.ps1'),
    'D:\home\site\wwwroot\scripts\precheck-avd.ps1',
    '/home/site/wwwroot/scripts/precheck-avd.ps1'
)
...
if (-not $scriptPath) {
    Push-OutputBinding -Name Response -Value ([HttpResponseContext]@{ StatusCode = 500; Body = '{"error":"Script precheck-avd.ps1 non trovato"}'; Headers = $corsHeaders })
    return
}
...
$outHtml = Join-Path $tempDir "avd_report_$subscriptionId.html"
$outJson = Join-Path $tempDir "avd_report_$subscriptionId.json"

if (Test-Path $outJson) { Remove-Item $outJson -Force }
if (Test-Path $outHtml) { Remove-Item $outHtml -Force }

try {
    Write-Host "Executing precheck-avd..."
    ...
    if (Test-Path $outJson) {
        ...
        Push-OutputBinding -Name Response -Value ([HttpResponseContext]@{ StatusCode = 500; Body = '{"error":"Report JSON AVD non generato"}'; Headers = $corsHeaders })
    }
} catch {
